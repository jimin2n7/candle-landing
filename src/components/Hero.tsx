...
export default function Hero() {
  ...
  return (
    <section ref={ref} className="relative h-screen flex items-center justify-center overflow-hidden bg-soot">
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-br from-soot via-bark-dark/30 to-soot" />
        <motion.div
          animate={{
          ...
          style={{
            background: 'radial-gradient(ellipse 80% 60% at 50% 50%, rgba(196, 168, 74, 0.15) 0%, transparent 70%)',
          }}
        />
      </motion.div>
      ...
      <motion.div style={{ y, opacity }} className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 w-full">
        ...
        >
          ...
          >
            ...
            <motion.span
              className="relative text-cream group-hover:text-soot transition-colors duration-500"
              whileHover={{ x: 5 }}
            >
              →
            </motion.span>
      ...
      <div className="hidden lg:block absolute left-12 top-1/2 -translate-y-1/2">
        ...
        >
          {['Fb', 'Ig', 'Tk'].map((s) => (
            <a key={s} href="#" className="text-[9px] tracking-[0.15em] text-cream/20 hover:text-cream/60 transition-colors duration-500 uppercase writing-mode-vertical">
              {s}
            </a>
          ))}
